...
namespace DATN1WEB.Controllers
{
    public class CategoriesController : Controller
    {
        ...
        public async Task<IActionResult> Index(string? search, string? hasImage, int page = 1, int pageSize = 5)
        {
            ...
            if (!await _permissionService.HasPermission(user, "Categories", "View"))
            {
                TempData["ErrorMessage"] = "Bạn không có quyền xem danh mục!";
        ...
        {
            var user = await _userManager.GetUserAsync(User);
            if (!await _permissionService.HasPermission(user, "Categories", "Create"))
            {
                TempData["ErrorMessage"] = "Bạn không có quyền thêm danh mục!";
                return RedirectToAction(nameof(Index));
            }
            return View();
        }
        ...
        {
            var user = await _userManager.GetUserAsync(User);
            if (!await _permissionService.HasPermission(user, "Categories", "Edit"))
            {
                TempData["ErrorMessage"] = "Bạn không có quyền sửa danh mục!";
                return RedirectToAction(nameof(Index));
            }
        ...
        {
            var user = await _userManager.GetUserAsync(User);
            if (!await _permissionService.HasPermission(user, "Categories", "Delete"))
            {
                TempData["ErrorMessage"] = "Bạn không có quyền xoá danh mục!";
                return RedirectToAction(nameof(Index));
            }
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            if (hasProducts)
            {
                TempData["ErrorMessage"] = "Danh mục vẫn còn sản phẩm, không thể xóa.";
                return RedirectToAction(nameof(Index));
            }
            ...
            catch (DbUpdateException ex)
            {
                var sqlEx = ex.GetBaseException() as SqlException;
                if (sqlEx?.Number == 547)
                    TempData["ErrorMessage"] = "Danh mục vẫn còn dữ liệu tham chiếu (sản phẩm). Không thể xóa.";
                else
                    TempData["ErrorMessage"] = "Không thể xoá danh mục do lỗi cơ sở dữ liệu.";
            }
            catch
