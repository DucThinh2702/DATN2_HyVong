...
namespace DATN1API.Controllers
{
    public class SizesController : Controller
    {
        ...
        public async Task<IActionResult> Index(string? search)
        {
            ...
            if (!await _permissionService.HasPermission(user, "Sizes", "View"))
            {
                TempData["ErrorMessage"] = "Bạn không có quyền xem kích cỡ!";
        ...
        {
            var user = await _userManager.GetUserAsync(User);
            if (!await _permissionService.HasPermission(user, "Sizes", "Create"))
            {
                TempData["ErrorMessage"] = "Bạn không có quyền thêm kích cỡ!";
                return RedirectToAction(nameof(Index));
            }
        ...
        {
            var user = await _userManager.GetUserAsync(User);
            if (!await _permissionService.HasPermission(user, "Sizes", "Edit"))
            {
                TempData["ErrorMessage"] = "Bạn không có quyền sửa kích cỡ!";
                return RedirectToAction(nameof(Index));
            }
        ...
        {
            var user = await _userManager.GetUserAsync(User);
            if (!await _permissionService.HasPermission(user, "Sizes", "Delete"))
            {
                TempData["ErrorMessage"] = "Bạn không có quyền xoá kích cỡ!";
                return RedirectToAction(nameof(Index));
            }
        ...
        public async Task<IActionResult> DeleteAjax(int id)
        {
            ...
            if (!await _permissionService.HasPermission(user, "Sizes", "Delete"))
            {
                return Json(new { success = false, message = "Bạn không có quyền xoá kích cỡ" });
            }
